...
Lis l’[ADR 001 sur la conservation des annulations](adr/001-conserver-annulations.md). Il relie le contexte, les options, le choix, ses conséquences et les tests correspondants.

![États conservés par la décision : une réservation confirmée peut être annulée, sans être recréée.](visuels/adr-annulation.svg)

Le [texte Mermaid de cet ADR](visuels/adr-annulation.mmd) conserve l’état du modèle à la date de la décision. Si une réactivation est ensuite acceptée, ajoute un nouvel ADR qui remplace celui-ci et relie les deux. Le diagramme de référence courante peut évoluer, tandis que le schéma historique garde son sens.

![Capture de la conférence montrant une revue de diagramme Mermaid dans un ADR.](visuels/sources/slide-adr-mermaid.png)

Samuel Rozé, Living Documentation, 2020. Capture fournie pour le cours, [crédits et conférence](visuels/credits.md).
...
Pour apprendre à afficher les descriptions et les types directement au survol dans VS Code, suivre la [manipulation JSDoc, TSDoc et TypeDoc](documentation-editeur.md). Elle fournit un appel JavaScript documenté, une variante TypeScript et un mauvais type à faire détecter. L’aide dans l’éditeur et la référence publiée réutilisent les informations placées près du code.
