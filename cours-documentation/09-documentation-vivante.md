...
À la fin de ce chapitre, tu sauras expliquer une règle par des exemples, relier un scénario à du code exécuté et choisir quelles informations générer. Le [kit JavaScript](../demo/README.md) fournit les fichiers nécessaires. Il représente les règles de réservation en mémoire, sans interface web, serveur HTTP ni base PostgreSQL.

![Le besoin métier alimente des exemples, des scénarios et du code. Les résultats et les décisions alimentent une documentation qui revient dans les discussions.](visuels/living.svg)
...
![Un besoin de réservation, deux règles, trois exemples et une question ouverte sur la réactivation.](visuels/example-mapping.svg)
...
Cette photo montre une collecte de besoins au WikiMuc, pas une séance BDD. Elle peut servir à discuter de la façon de conserver le résultat d’un atelier. Crédit : Jan Dittrich (WMDE), [source et licence](visuels/credits.md).
...
Le [fichier complet de scénarios](../demo/features/reservations.feature) couvre la dernière place, l’atelier complet, le retour après annulation et la place libérée pour une autre personne.
...
Dans [les définitions complètes](../demo/features/steps/reservations.mjs), un Before recrée un historique vide pour chaque scénario. Les fonctions ordinaires donnent accès au contexte du scénario par this. Les assertions comparent le résultat à l’attendu.
...
La documentation peut aussi être consultée directement dans l’éditeur. Le [guide JSDoc, TSDoc, TypeDoc et aide au survol](documentation-editeur.md) montre comment faire apparaître le contrat d’une fonction pendant son utilisation, puis générer une référence à partir de ses commentaires. Les types, les explications et les tests se complètent.

Les [commentaires JSDoc](../demo/src/reservations.mjs) précisent les paramètres, les effets et les erreurs de la fonction. La commande suivante produit leur référence HTML :
...
![Les imports de la démonstration relient le programme de lancement aux réservations, puis aux statuts.](visuels/dependances.svg)
...
Conserve les vues C4 pour donner le contexte aux lecteurs. Utilise les vues extraites pour explorer les détails techniques. La [sélection d’outils](outils-et-sources.md) compare Mermaid, Structurizr, LikeC4, dependency-cruiser, Mocodo et SchemaSpy.
...
Le [hook fourni](../scripts/mkdocs_hook.py) insère des fichiers explicitement sélectionnés dans les pages, au moment de la construction. Les [exemples complets](exemples.md) l’utilisent. Il n’analyse pas les annotations JavaScript : JSDoc s’en charge séparément. Il ne parcourt pas aveuglément le disque.
...
Le corrigé doit suivre la décision prise par le groupe. Pour une première démonstration plus courte, le [guide d’animation](../guide-animation.md) fournit une régression préparée et réversible sur la capacité.
...
[Retour au sommaire](../documenter-son-projet-web-atelier-5h.md)
