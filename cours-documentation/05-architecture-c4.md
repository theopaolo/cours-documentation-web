...
[Source Mermaid de la vue de contexte](visuels/c4-contexte.mmd).

![Vue C4 de conteneurs avec navigateur, serveur et PostgreSQL.](visuels/c4-conteneurs.svg)

[Source Mermaid de la vue de conteneurs](visuels/c4-conteneurs.mmd). Un conteneur C4 désigne ici une application ou un stockage. Il ne signifie pas nécessairement un conteneur Docker.

Mermaid propose bien une syntaxe C4. Sa [documentation officielle](https://mermaid.js.org/syntax/c4.html) la signale encore comme expérimentale lors de la vérification de septembre 2026. Le kit fixe la version du moteur et conserve le SVG. Pour un ensemble de vues issues d’un modèle commun, examine [Structurizr ou LikeC4](outils-et-sources.md).

## Montrer un échange et un cas d’échec

![Séquence illustrative d’une demande de réservation acceptée ou refusée.](visuels/reservation-sequence.svg)

[Source de la séquence](visuels/reservation-sequence.mmd). Ce dessin propose un comportement à discuter. La gestion transactionnelle des accès concurrents reste à concevoir dans une application réelle.
...
Un diagramme Mermaid écrit à la main reste un modèle à maintenir. Le [graphe des imports du kit](visuels/dependances.svg), lui, est extrait du code. Il ne permet pas de déduire tous les utilisateurs, services externes ou raisons des choix d’architecture.
